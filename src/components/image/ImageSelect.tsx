...
const ImageSelect = ({ onImageSelectFtn, selectedImageId }: Props) => {
  ...
  const addSelection = (element: any) => {
    const isProjectImageElement = element.classList.contains(styles['project-image'])
  ...
  if (images) {
    for (const image of images) {
      imageList.push(
        <span
          key={ image.id }
          className={ styles['project-image'] + (selectedImageId === image.id ? ' ' + styles['project-image-selected'] : '') }
